...
def Bottleneck(x, size, downsampe=False):
    ...
    if downsampe:
        ...
        residual = BatchNormalization(momentum=0.99)(residual)
        ...
        if i < num_in:
            if in_channels[i] != out_channels[i]:
                ...
                residual = BatchNormalization(
                    epsilon=1e-5, momentum=0.99)(residual)
...
def HRNet(input_size=(224, 224, 3), cls_num=3):
    """Deep High-Resolution Representation Learning for Human Pose Estimation Network"""
    channels_2 = [32, 64]
    ...
    up1 = UpSampling2D(interpolation='bilinear')(st4[0])
    up1 = conv(up1, 32, 3)
    up1 = BatchNormalization(momentum=0.99)(up1)
    up1 = Activation('relu')(up1)
    up2 = UpSampling2D(interpolation='bilinear')(up1)
    up2 = conv(up2, 32, 3)
    up2 = BatchNormalization(momentum=0.99)(up2)
    up2 = Activation('relu')(up2)
    final = conv(up2, cls_num, 1, padding_='valid', activation='softmax')
